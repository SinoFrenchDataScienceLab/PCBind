...
from components.AttentionUtils import permute_final_dims, Attention
from components.ModelUtils import Transition, InteractionModule


class TrioformerBlock(nn.Module):
    def __init__(self, node_hidden_dim, pair_hidden_dim, dist_hidden_dim, use_column_attention=True):
        super(TrioformerBlock, self).__init__()
        self.use_column_attention = use_column_attention
        self.dist_hidden_dim = dist_hidden_dim
        self.pair_hidden_dim = pair_hidden_dim
        self.triangle_block_row = RowTriangleAttentionBlock(pair_hidden_dim, dist_hidden_dim)
        if self.use_column_attention:
            self.triangle_block_column = RowTriangleAttentionBlock(pair_hidden_dim, dist_hidden_dim)

        self.p_attention_block = RowAttentionBlock(node_hidden_dim, pair_hidden_dim)
        self.c_attention_block = RowAttentionBlock(node_hidden_dim, pair_hidden_dim)
        self.p_transition = Transition(node_hidden_dim, 2)
        self.c_transition = Transition(node_hidden_dim, 2)
        self.pair_transition = Transition(pair_hidden_dim, 2)
        self.inter_layer = InteractionModule(node_hidden_dim, pair_hidden_dim, 32)
...
class RowTriangleAttentionBlock(nn.Module):
    ...
    def __init__(self, pair_hidden_dim, dist_hidden_dim, attention_hidden_dim=32, no_heads=4, dropout=0.1):
...
class RowAttentionBlock(nn.Module):
    ...
    def __init__(self, node_hidden_dim, pair_hidden_dim, attention_hidden_dim=32, no_heads=4, dropout=0.1):
